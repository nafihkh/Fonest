    ...
    <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
      ...
      <div className="p-3">
        <Link to={`/product/${product.id || product._id}`}>
          <h3 className="line-clamp-2 min-h-[40px] text-[13px] font-semibold text-gray-900">
            {product.name}
          </h3>
        </Link>
        ...
        {quantity > 0 ? (
        <div className="mt-3 flex h-10 items-center justify-between rounded-xl border border-gray-200 bg-gray-50 px-2">
          <button
            onClick={handleDecrease}
            disabled={isBusy}
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-gray-800 shadow-sm disabled:opacity-50"
          >
            <Minus size={14} />
          </button>
        ...
        <button
          ...
          className={`mt-3 flex h-10 w-full items-center justify-center gap-2 rounded-xl text-[12px] font-semibold transition ${
            product.stock === 0
              ...
              : "bg-gray-900 text-white hover:bg-black disabled:opacity-70"
          }`}
        >
          {isBusy ? (
            <>
